...
%
%   Simulates the attitude, orbit, thermal, power and communications
%   subsystems.
%
%   This script makes use of the following Matlab functions included with the
%   tutorial:
%
%   Link.m
%   ThermalOptical.m
...
if( strcmp(lower(simName),'solstice' ))
    beta     = 23.44*pi/180; % Summer solstice
elseif ( strcmp(lower(simName),'equinox' ))
    beta = 0;
else
    error(sprintf('%s not known',simName) )
end
...
d.eMax   = 1000; % Maximum battery charge (J)
d.m      = 1000; % Mass (kg)
...
q        = [0.0;0;0]; % Small angles from local vertical (rad)
